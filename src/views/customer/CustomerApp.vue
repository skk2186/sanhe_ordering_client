  ...
  <div class="customer-app">
    ...
    <div class="status-bar">
      <div class="status-left">
        ...
        <span>{{ $t('cart.table') }} {{ tableNumber }}</span>
        </div>
      ...
      <span class="member-points">{{ $t('customer.points') }}：{{ memberInfo.points }}</span>
        </div>
    ...
    <div v-if="cartItems.length > 0" class="cart-bar">
      ...
      <div class="cart-text">{{ $t('customer.viewCart') }}</div>
        </div>
      </div>
      <el-button
    ...
    <div v-if="plateCount > 0" class="plate-progress">
      ...
      {{ $t('customer.plateHint', { current: plateCount, remaining: 5 - (plateCount % 5) }) }}
      </div>
    </div>
...
const handleGachaReward = (reward) => {
  ElMessage.success(t('customer.rewardEarned', { name: reward.name }))
