  ...
  >
    ...
    <div class="dish-image-container">
      ...
      <div class="sold-out-overlay" v-if="item.status === 'SOLD_OUT'">
        <span class="sold-out-text">{{ $t('common.soldOut') }}</span>
      </div>
...
const addToCart = async () => {
  ...
  try {
    ...
    ElMessage.success(t('menu.addedToCart', { name: props.item.name }))
    emit('add-to-cart', props.item)
  } catch (error) {
    ElMessage.error(t('menu.addFailed'))
  } finally {
...
const handleQuantityChange = async ({ item, quantity, action }) => {
  try {
    ...
    ElMessage.error(action === 'increase' ? t('menu.addFailed') : t('menu.removeFailed'))
  }
}
