...
/* Midnight Station turns the assistant into a night-duty service desk. */
[data-theme="midnight-station"] .assistant-recommendation-rail {
  --assistant-panel-width: 286px;
  --rail-gap: 12px;
...
[data-theme="midnight-station"] {
  .recommendation-lane__header {
    color: #e5d7bc; font-size: clamp(14px, .8vw, 18px);
    strong { color: #f2c978; font-size: inherit; }
    span, .customer-transcript { color: #e5d7bc; }
  }
  .recommendation-card {
    height: 104px;
    ...
    border-radius: 3px;
    box-shadow: 2px 3px 0 rgba(2, 10, 9, .32);
    &:hover, &:focus-visible { border-color: #f2c978; box-shadow: 2px 4px 0 rgba(2, 10, 9, .4); transform: translateY(-1px); }
    &:focus-visible { outline-color: #f2c978; }
  }
  .recommendation-number { background: #28483d; border-radius: 3px; color: #fff1c8; }
  ...
  .assistant-halo { left: 20px; right: 20px; bottom: 11px; height: 18px; background: rgba(217, 155, 74, .34); border-color: rgba(242, 201, 120, .72); }
  .sound-level i { background: #d99b4a; }
  .is-error .assistant-bubble { border-color: #8c3d34; }
  .is-listening .assistant-bubble::before { content: '● '; color: #718e70; }
  .is-processing .assistant-bubble::before { content: '◆ '; color: #d99b4a; }
  .is-result .assistant-bubble::before { content: '✓ '; color: #4f7056; font-weight: 900; }
}
