      ...
      <div v-if="cartStore.isEmpty" class="empty-cart">
        <el-empty description="购物车是空的">
          <div class="empty-actions">
            <p class="empty-tip">快去选择您喜欢的美食吧！</p>
      ...
      <div class="coupon-list">
        ...
        >
          <div class="coupon-info">
            ...
            <span class="coupon-amount">减{{ formatPrice(coupon.amount) }}</span>
          </div>
        </div>
      </div>
...
const selectCoupon = (coupon) => {
  if (cartStore.totalAmount >= coupon.minAmount) {
    ...
    ElMessage.warning(`此优惠券需要满${formatPrice(coupon.minAmount)}才能使用`)
  }
}
...
const applyCoupon = () => {
  if (selectedCoupon.value) {
    ...
    ElMessage.success(`已使用优惠券：${selectedCoupon.value.name}`)
  }
